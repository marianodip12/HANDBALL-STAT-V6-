...
export const computeTeamStats = (eventos, equipo) => {
  ...
  // Goals by type
  const porTipoGol = Object.values(TIPOS_GOL).map(t=>({
...
export const computePlayerStats = (jugador, eventos) => {
  ...
  return {
    ...
    porTipoGol:Object.values(TIPOS_GOL).map(t=>({tipo:t,goles:soloGoles(evs).filter(e=>e.tipoGol===t).length})),
    porDistancia:Object.values(DISTANCIAS).map(d=>({...buildStatsRow(d,evs.filter(e=>e.distancia===d)),distancia:d})),
    shotMap:tiros.filter(e=>e.posX!=null&&e.posY!=null).map(e=>({x:e.posX,y:e.posY,gol:e.tipoEvento==='gol'})),
